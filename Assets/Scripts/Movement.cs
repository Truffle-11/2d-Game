...
public class Movement : NetworkBehaviour
{
    ...
    {
        // Who called the RPC (this is the real dead client)
        ulong senderId = rpcParams.Receive.SenderClientId;

        // Owner of this NetworkObject
        ulong ownerId = NetworkObject != null ? NetworkObject.OwnerClientId : senderId;

        // Always trust the sender
        ulong deadClientId = senderId;

        if (ownerId != senderId)
        {
            Debug.LogWarning(
                "DieServerRpc mismatch: senderId=" + senderId +
                " ownerId=" + ownerId + " using senderId."
            );
        }
    ...
    // Called when you return to lobby and get revived (server authoritative)
